...
Ce modèle a déjà été utilisé sur plusieurs projets importants dont au sein de grandes organisations. Il s'enrichit régulierement.

NOTE: Nous utilisons le terme 'DA' et pas 'DAT' ou terme similaire car chaque organisation possède sa terminologie mais surtout, 'technique' (comme 'fonctionnel' d'ailleurs) est un terme équivoque (qu'est ce qui n'est pas "technique" dans l'IT ?).
