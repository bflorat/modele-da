...
//🏷{"id": "1d3ec06e-d63d-4a61-bb4e-437358064687", "labels": ["niveau::intermédiaire", "niveau_detail::détaillé"]}
### Contraintes puissance de calcul

TIP: Lister ici les éventuelles contraintes liées à la puissance de calcul (CPU, GPU, TPU)
[Exemple]
====
...
====
Exemple 3 : Sur TPU v5e, un modèle d’inférence reçoit une slice dédiée de 8 chips (≈128 GB HBM total). Une slice n’héberge pas plus de 1 modèle ‘latency critical’.
====

//🏷{"id": "ec6a420b-c284-442a-93fd-edc3d45ed00a", "labels": ["niveau::intermédiaire", "niveau_detail::détaillé"]}
### Contraintes mémoire
...
[TIP]
====
Lister ici les besoins en stockage de chaque module une fois l’application arrivée à pleine charge (volumétrie à cinq ans par exemple).
...
====
Exemple de volumétrie statique :
...
Si l'application est déployée sur un cloud de type serverless, la charge sera absorbée dynamiquement. Il convient néanmoins d'estimer le surcoût et de fixer des limites de consommation pour concilier budget et qualité de service.
====

.Exemple : estimation volumétrie dynamique de l'opération REST `GET Detail` de l'application MIEL
|===
|Taux maximal d’utilisateurs connectés en même temps en pic annuel | S1 x F1 x 0.2 = 8K sur 1h
...
Exemple de profils de sollicitation :
[cols='3e,1e,1e,1e']
|===
...
//🏷{"id": "e30801f9-af5b-49f3-bdca-214aae776336", "labels": ["niveau_detail::détaillé"]}
### Dimensionnement du stockage externe hors SAN

[TIP]
====
Le stockage externe hors SAN concerne du stockage fichier sur un système de fichier distribué (NFS, CIFS, WebDav, …) ou un stockage objet (Swift, S3, etc.).
