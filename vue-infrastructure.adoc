...
*Disponibilité exigée par plage de fonctionnement*

La disponibilité ne s'exige pas uniformément : une interruption de nuit et la
même interruption en heures ouvrées n'ont pas les mêmes conséquences. Le niveau
se cote donc *par plage*, chaque plage renvoyant à celles définies dans les
contraintes de disponibilité du SI.

.Exemple
[cols="1e,2,1,3", options="header"]
|===
| N° Plage | Indisponibilité maximale admissible | Niveau | Justification

| _1_
| _24 h, maximum 7 fois par an_
| _D2_
| _Nuits et week-ends. Aucune instruction en cours, aucun accès usager attendu._

| _2_
| _1 h, 8 fois dans l'année_
| _D4_
| _Heures ouvrées. L'interruption bloque l'accueil au guichet et l'instruction des dossiers._

| _3_
| _4 h, 8 fois dans l'année_
| _D3_
| _Samedi matin. Activité réduite, mais service ouvert au public._
|===

[PRE-FILLED]
====
.Disponibilité exigée par plage
[cols="1e,2,1,3", options="header"]
|===
| N° Plage | Indisponibilité maximale admissible | Niveau | Justification
...
[TIP]
====
*La colonne « Justification » est la plus utile du tableau.* Devoir écrire
pourquoi une plage exige tel niveau conduit souvent le porteur de projet à
constater que la nuit et le week-end ne réclament pas ce qu'il avait demandé.
C'est un outil de modération plus efficace que n'importe quel avertissement sur
les coûts.

Si toutes les plages portent le même niveau, c'est le signe qu'il n'y a en
réalité qu'une seule plage : simplifier plutôt que de dupliquer des lignes
identiques.
====

*Perte de données tolérée*

Le RPO se cote *une seule fois pour l'ensemble du système* : la tolérance à la
perte est une propriété des données, non de l'horaire. Un RPO variant au cours
de la journée serait au demeurant ingérable techniquement.

.Exemple
[cols="2,1,3", options="header"]
|===
| RPO exigé | Niveau | Justification

| _1 h_
| _D3_
| _Une heure de saisie perdue reste ressaisissable par les agents. Au-delà, les dossiers déposés par les usagers seraient définitivement perdus._
|===
...
*Niveau retenu pour le système*

Le niveau de disponibilité du système est *le plus élevé* entre le maximum des
plages et le niveau issu du RPO. Dans l'exemple ci-dessus : maximum des plages
D4, RPO D3, donc *D4*.
...
[TIP]
====
*Choisir le niveau RPO en partant des scénarios de restauration*, et non du seul
...
pénalisant qu'une interruption longue sans perte. C'est la raison pour laquelle
les deux sont cotés séparément ci-dessus.

Si les deux axes divergent fortement et durablement, préférer une cotation en
deux valeurs (ex. : `RTO D4 / RPO D3`) plutôt qu'un chiffre unique qui masquerait
l'écart.
====
