...
Ce tableau couvre les trois catégories reconnues comme facteurs :
connaissance, possession, inhérence.
 
[cols="2,1,1,1,3,3,2", options="header"]
|===
| Facteur | Catégorie | Force | Résistance au phishing | Description | Faiblesses / Risques | Norme
...
| Réutilisation entre services, fuite par bases compromises, hameçonnage, force brute. Insuffisant seul.
| NIST SP 800-63B
...
| Moyenne
| Non
| Suite de mots facile à mémoriser (ex. : `CorrectHorseBatteryStaple`).
| Meilleure entropie qu'un mot de passe court, mais aussi vulnérable au hameçonnage et au rejeu.
| NIST SP 800-63B
...
| Très faible
| Non
| Question personnelle (ex. : nom du premier animal).
| Réponses devinables ou publiquement accessibles. Déconseillé.
| Déconseillé par NIST SP 800-63B

| Code PIN
| Connaissance
| Faible seul
| Non
| Code numérique court (4 à 8 chiffres).
| Espace de recherche réduit. N'a de valeur que combiné à un dispositif limitant les tentatives (carte, TPM, élément sécurisé).
| —
...
| Faible
| Non
| Question portant sur une information interne (ex. : montant de la dernière facture).
| Partageable, mémorisation difficile, souvent accessible à des tiers légitimes. *Déconseillée comme facteur d'authentification* ; son usage légitime est
l'appariement lors de l'enrôlement initial, quand aucun autre élément partagé
n'existe.
| —
...
| Faible
| Non
| Lien à usage unique envoyé sur l'adresse déclarée.
| Sécurité entièrement dérivée de celle de la boîte mail. Interception, transfert, hameçonnage.
| —
...
| Faible
| Non
| Code à usage unique envoyé par message texte.
| Détournement de carte SIM (_SIM swapping_), interception au niveau de la signalisation inter-opérateurs (SS7, Diameter), dépourvue d'authentification entre réseaux, transfert automatique. Déconseillé pour les usages sensibles.
| —
...
| Forte
| Non
| Code temporaire calculé à partir d'un secret partagé et de l'heure.
| Rejeu possible par proxy d'interception en temps réel. Perte du terminal. Secret extractible si le terminal est compromis.
| RFC 6238
...
| Mêmes limites que le TOTP face au proxy d'interception. Coût, logistique, perte.
| RFC 4226 (HOTP), RFC 6238 (TOTP) ou protocole propriétaire

| Notification push
| Possession
| Moyenne
| Partielle
| Demande de validation envoyée à un terminal enrôlé.
| Vulnérable au harcèlement de notifications (_MFA fatigue_) si l'utilisateur peut valider sans contexte. La correspondance de nombre affiché réduit fortement ce risque.
| Propriétaire
...
| Forte
| Partielle
| Paire de clés asymétriques, la clé privée restant sur le poste client.
| Clé privée souvent stockée en clair sur le disque. Absence fréquente de phrase de passe. Gestion du cycle de vie et de la révocation.
| RFC 4252
...
| Moyenne
| Oui
| Certificat X.509 et clé privée conservés dans un magasin logiciel (navigateur,
système, fichier PKCS#12). Souvent déployé automatiquement sur les terminaux
gérés (MDM, jonction au domaine) pour authentifier le poste en plus de
l'utilisateur.
| Clé privée exportable et copiable. Exfiltration par code malveillant. La
possession exclusive ne peut être démontrée.
| RFC 5280

| Carte à puce / élément sécurisé
| Possession + Connaissance (PIN)
| Très forte
| Oui
| Certificat X.509 dont la clé privée est générée dans un composant sécurisé et non extractible ; les opérations cryptographiques sont réalisées dans le composant, débloqué par un code PIN.
| Coût. Lecteur et intergiciel requis. PKI à exploiter (révocation, renouvellement). Blocage du PIN, perte de la carte.
| ISO/IEC 7816, PKCS#11, eIDAS (QSCD)

| Clé matérielle FIDO2
| Possession (+ PIN ou biométrie)
| Très forte
| Oui
| Défi-réponse asymétrique, la signature étant liée à l'origine du site. La clé privée est confinée dans un composant externe dédié, indépendant du poste et transportable d'une machine à l'autre.
| Coût unitaire (~20 à 50€) et logistique de distribution. Impose l'enrôlement d'une clé de secours. Support applicatif encore inégal.
...
| Très forte
| Oui
| Même protocole que la clé matérielle, mais la clé privée est confinée dans l'élément sécurisé de l'appareil lui-même (Secure Enclave, TPM).
| Aucun matériel à distribuer, mais la clé disparaît avec l'appareil : impose l'enrôlement d'un second dispositif. Dépendance à la plateforme.
| W3C WebAuthn
...
| Forte
| Oui
| Clé privée WebAuthn générée sur un appareil puis répliquée, chiffrée, vers les autres appareils de l'utilisateur via son gestionnaire de justificatifs (Apple, Google, Microsoft ou gestionnaire tiers).
| La clé quitte l'appareil : la possession n'est plus exclusive. La sécurité effective devient celle du compte de synchronisation et surtout de sa procédure de récupération, souvent ramenée à un code SMS ou à un lien par e-mail. Portabilité limitée entre écosystèmes.
| W3C WebAuthn
...
| Variable
| Sans objet
| Empreinte digitale, reconnaissance faciale, iris, voix.
| Non révocable en cas de compromission. Taux d'erreur et attaques par présentation. Enjeux de vie privée et de conformité RGPD.
| ISO/IEC 30107
|===
...
La colonne _Force_ traduit la difficulté à compromettre le facteur (deviner,
voler, dupliquer). La colonne _Résistance au phishing_ traduit la capacité à
résister à un attaquant qui s'interpose en temps réel entre l'utilisateur et le
service légitime (_adversary-in-the-middle_).
 
Un TOTP est fort mais se rejoue : l'utilisateur saisit son code sur un site
contrefait qui le relaie immédiatement au vrai service. À l'inverse, un
certificat logiciel est moyennement fort — la clé est copiable — mais résistant
au phishing, car la preuve est liée cryptographiquement à l'origine.
 
Trois mécanismes cumulent aujourd'hui les deux propriétés au plus haut niveau : la clé matérielle FIDO2, la passkey liée à l'appareil (non synchronisée) et l'authentification par certificat sur élément sécurisé (carte à puce). Ils partagent le même principe — la clé privée ne quitte jamais un composant sécurisé. 

Pour tout accès réellement sensible, c'est de ce côté qu'il faut regarder.
====
 
[NOTE]
====
La *biométrie* est rarement un facteur transmis au service : dans les usages
courants (téléphone, ordinateur portable), elle déverrouille localement un
élément sécurisé qui porte le véritable facteur. La force et la résistance au
phishing sont alors celles du dispositif déverrouillé, pas celles de la mesure
biométrique.
====
 
*Signaux de contexte : tableau comparatif*
 
Les éléments ci-dessous ne prouvent pas une identité et ne constituent donc pas
des facteurs d'authentification. Ils servent à *moduler* l'exigence
d'authentification en fonction de la situation d'exécution — démarche appelée
authentification adaptative ou _risk-based_.
 
[cols="2,1,3,3,3", options="header"]
|===
| Signal | Catégorie | Ce qu'il permet de détecter | Modulation typique | Limites
 
| Adresse IP ou plage réseau
| Localisation
| Accès depuis un réseau non prévu (Internet public au lieu de l'intranet ou du VPN).
| Exiger un facteur supplémentaire hors du réseau de confiance ; restreindre certaines opérations sensibles au réseau interne.
| Usurpation d'adresse. Ne distingue pas les utilisateurs d'un même réseau. Contraignant en mobilité et en télétravail.
 
| Géolocalisation
| Localisation
| Connexion depuis un pays ou une région inhabituelle pour l'utilisateur.
| Élever le niveau d'authentification ; bloquer les pays hors périmètre d'activité.
| Contournable par VPN ou relais. Précision faible sur IP. Faux positifs fréquents.
 
| Voyage impossible
| Localisation
| Deux connexions géographiquement incompatibles avec le temps écoulé entre elles.
| Réauthentifier, invalider la session en cours, alerter le SOC.
| Faux positifs liés aux VPN d'entreprise et aux relais de confidentialité.
 
| Empreinte de terminal
| Localisation
| Accès depuis un terminal jamais observé pour ce compte.
| Exiger une vérification supplémentaire lors du premier usage d'un terminal.
| Facilement falsifiable. Se dégrade en simple identifiant persistant. Enjeux RGPD liés au traçage.
 
| Plage horaire d'activité
| Comportement
| Connexion en dehors des horaires habituels de l'utilisateur ou du service.
| Élever l'exigence ; restreindre les opérations irréversibles aux heures ouvrées.
| Faux positifs en astreinte, en horaires décalés ou sur plusieurs fuseaux.
 
| Dynamique de saisie
| Comportement
| Écart entre le rythme de frappe observé et le profil appris.
| Alimenter un score de risque ; déclencher une réauthentification au-delà d'un seuil.
| Nécessite une phase d'apprentissage. Peu fiable isolément. Sensible au changement de clavier ou de terminal.
 
| Habitudes d'usage
| Comportement
| Séquence d'actions, volumétrie ou vitesse de navigation inhabituelles.
| Alimenter un score de risque ; limiter le débit ; alerter.
| Dérive du profil dans le temps. Difficulté à expliquer une décision de blocage à l'utilisateur.
|===
 
[WARNING]
====
Un signal de contexte ne compte *jamais* comme second facteur. Une
authentification multifacteur doit combiner des éléments d'au moins deux des
trois catégories reconnues (connaissance, possession, inhérence). Un mot de
passe complété d'une restriction d'adresse IP reste une authentification à un
seul facteur.
 
Corollaire : ces signaux peuvent *élever* l'exigence d'authentification, jamais
l'abaisser en deçà du minimum requis pour l'opération concernée.
====
...
|===
| Niveau d'accès | Exemple | Combinaison minimale exigée | Facteur anti-phishing | Signaux de contexte
 
| Accès public
| Compte sur un site d'information, forum, newsletter.
| Un facteur : phrase de passe, ou lien à usage unique par e-mail.
| Non
| <aucun>
 
| Accès personnel sensible
| Compte bancaire, espace assuré, portail usager, dossier médical.
| Deux facteurs de deux catégories distinctes, dont *au moins un résistant au phishing* : phrase de passe *et* passkey (synchronisée acceptée).
| Oui
| Terminal inconnu, voyage impossible : exiger une réauthentification.
 
| Accès interne
| Application métier, données RH, intranet d'un agent.
| Certificat sur carte à puce *ou* passkey liée à l'appareil, déverrouillé par code PIN ou biométrie.
| Oui
| Réseau d'origine, posture de conformité du poste.
 
| Accès distant
| VPN, bureau à distance, bastion.
| Idem accès interne, *et* authentification du poste par certificat déployé sur la flotte gérée.
| Oui
| Géolocalisation, plage horaire, terminal non géré : refuser.
 
| Accès à privilèges
| Compte `root`, administration d'un hyperviseur, console cloud.
| Clé matérielle FIDO2 *ou* carte à puce, avec code PIN, depuis un poste d'administration dédié. Accès via bastion et élévation limitée dans le temps.
| Oui
| Plage horaire, voyage impossible. Session enregistrée et double validation pour les opérations irréversibles.
|===
 
[WARNING]
====
Dès que la compromission d'un compte a des conséquences financières, juridiques
ou opérationnelles, au moins un facteur résistant au phishing est nécessaire :
WebAuthn (clé matérielle ou passkey) ou certificat sur élément sécurisé.
====
...
|===
| Cas d'utilisation | Facteurs exigés | Facteur résistant au phishing | Signaux de contexte

| _Création de l'espace usager et appariement au dossier existant_
| _Phrase de passe, et vérification du numéro de dossier accompagné du montant
de la dernière notification._
| _Non_
| _Aucun. Blocage après 3 tentatives d'appariement infructueuses._

| _Consultation de son dossier par l'usager_
| _Phrase de passe et passkey (variante synchronisée acceptée)._
| _Oui_
| _Terminal inconnu ou voyage impossible : réauthentification complète._

| _Instruction d'un dossier par un agent_
| _Carte agent (certificat sur carte à puce) déverrouillée par code PIN._
| _Oui_
| _Poste non conforme au référentiel de la flotte gérée : accès refusé._

| _Accès d'un agent en télétravail_
| _Carte agent et code PIN, et certificat de poste déployé par le MDM._
| _Oui_
| _Connexion hors des pays autorisés : accès refusé. Hors plage 7h-20h :
notification au responsable._

| _Administration des serveurs applicatifs_
| _Clé matérielle FIDO2 avec code PIN, depuis un poste d'administration dédié.
Accès via bastion et élévation de privilèges limitée à 4 heures._
| _Oui_
| _Hors plage ouvrée : double validation par un second administrateur. Session
enregistrée._

| _Échange automatisé avec l'organisme partenaire (API)_
| _Authentification mutuelle TLS par certificat client délivré par la PKI
interne, renouvellement annuel._
| _Sans objet_
| _Restriction aux plages d'adresses déclarées par le partenaire._
|===
====

[PRE-FILLED]
====
Le tableau ci-dessous indique les *facteurs d'authentification requis* selon le
