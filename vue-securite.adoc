...
* … et les journaux applicatifs, par exemple via un fichier de log : `[INFO] 2016/12/23 11:14 [Agent X] Appel du service consulter` qui sont de niveau technique. 
...
====
====
Exemple 5 : Les journaux contenant le tag `[PREUVE]` et issu de l’ensemble des modules seront centralisés via le système de centralisation de log Elastic Search puis insérés avec traitement Logstash de façon journalière vers l'index Elastic `preuves`.
====
...
* Les outils de SIEM (Security Information and Event Management) qui analysent les journaux issus de sources variées.
...
* Les outils et méthodologies d'analyse forensique permettant d'analyser les journaux pour comprendre les chemins d'une attaque et construire un post-mortem.
...
|X
a|
* __Échappement des journaux avant de les transmettre à log4j__
* __Vérification des outils de consultation de journaux__
