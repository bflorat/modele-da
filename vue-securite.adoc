...
| Partageable, mémorisation difficile, souvent accessible à des tiers légitimes.
| —
...
| Détournement de carte SIM (_SIM swapping_), interception SS7, transfert automatique. Déconseillé pour les usages sensibles.
| —
...
| Moyenne
| Oui
| Certificat X.509 et clé privée conservés dans un magasin logiciel (navigateur, système, fichier PKCS#12).
| Clé privée exportable et copiable. Exfiltration par code malveillant. La possession exclusive ne peut être démontrée.
| RFC 5280
...
| Très forte
| Oui
| Clé privée générée dans un composant sécurisé et non extractible ; les opérations cryptographiques sont réalisées dans le composant, débloqué par un code PIN.
...
| Très forte
| Oui
| Authentification par défi-réponse asymétrique, la signature étant liée à l'origine du site.
| Coût. Nécessite un enrôlement et une clé de secours. Support applicatif encore inégal.
| FIDO2, W3C WebAuthn
...
| Forte
| Oui
| Justificatif WebAuthn synchronisé entre les appareils de l'utilisateur via son compte plateforme.
| La sécurité est reportée sur le compte de synchronisation. Récupération de compte souvent plus faible que le facteur lui-même.
| W3C WebAuthn
...
| Moyenne
| Non
| Terminal préalablement déclaré et porteur d'une clé ou d'un certificat le liant au compte.
| Ne vaut que si la liaison est cryptographique. À distinguer de la simple empreinte de terminal, qui est un signal de contexte (voir tableau suivant).
| Propriétaire
...
Seuls deux mécanismes cumulent aujourd'hui les deux propriétés : *WebAuthn /
FIDO2* et l'*authentification par certificat sur élément sécurisé* (carte à
puce). Pour tout accès réellement sensible, c'est de ce côté qu'il faut
regarder.
====
