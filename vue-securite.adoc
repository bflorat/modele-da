...
[cols="1,1,1,1,5,5,1", options="header"]
|===
| Facteur | Catégorie | Force | Résistance au phishing | Description | Faiblesses / Risques | Normes
...
| Moyenne
| Non
| Suite de mots facile à mémoriser (ex. : `cheval juste pile agrafe`).
...
| Partageable, mémorisation difficile, souvent accessible à des tiers légitimes. *Déconseillée comme facteur.* Usage légitime : l'appariement lors de l'enrôlement initial, quand aucun élément partagé n'existe.
| —
...
| Détournement de carte SIM (_SIM swapping_). Interception de la signalisation entre opérateurs (SS7, Diameter), dépourvue d'authentification entre réseaux. Transfert automatique vers un terminal tiers. Déconseillé pour les usages sensibles.
| —
...
| Moyenne
| Oui
| Certificat X.509 et clé privée conservés dans un magasin logiciel (navigateur, système, fichier PKCS#12). Souvent déployé sur les terminaux gérés (MDM, domaine) pour authentifier le poste en plus de l'utilisateur.
| Clé privée exportable et copiable. Exfiltration par code malveillant. Possession exclusive non démontrable.
| RFC 5280
...
| Très forte
| Oui
| Certificat X.509 dont la clé privée est générée dans un composant sécurisé et non extractible. Les opérations sont réalisées dans le composant, débloqué par un code PIN.
...
| Très forte
| Oui
| Défi-réponse asymétrique, la signature étant liée à l'origine du site. Clé privée confinée dans un composant externe dédié, transportable entre machines.
...
| Très forte
| Oui
| Même protocole que la clé matérielle. Clé privée confinée dans l'élément sécurisé de l'appareil (Secure Enclave, TPM).
| Aucun matériel à distribuer. La clé disparaît avec l'appareil. Impose un second dispositif enrôlé. Dépendance à la plateforme.
| W3C WebAuthn
...
| Forte
| Oui
| Clé privée WebAuthn générée sur un appareil, puis répliquée chiffrée vers ses autres appareils. La réplication passe par son gestionnaire de justificatifs (Apple, Google, Microsoft ou tiers).
| La clé quitte l'appareil : possession non exclusive. La sécurité devient celle du compte de synchronisation, et de sa procédure de récupération, souvent un code SMS ou un lien e-mail. Portabilité limitée entre écosystèmes.
| W3C WebAuthn
...
| Non révocable si compromise. Taux d'erreur et attaques par présentation. Enjeux de vie privée et de conformité RGPD.
| ISO/IEC 30107
|===
