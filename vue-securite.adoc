...
| Clé publique SSH                 | Possession      | Forte   | Paire de clés asymétriques (publique/privée) pour l'authentification. | Perte/vol de la clé privée = accès compromis. Gestion complexe. | Accès aux serveurs (SSH, Git). | RFC 4251
...
| Clé matérielle (YubiKey)         | Possession      | Très forte | Clé physique (USB/NFC) pour une authentification forte (FIDO2). | Coût (~50€). Nécessite un appareil compatible. | Accès admin, comptes sensibles, crypto-monnaies. | FIDO2
