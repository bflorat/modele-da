...
TIP: En architecture, l'incertitude doit être affichée clairement pour aider à la comprehension des choix et de la situation courante.
...
//🏷{"id": "382fd086-f48e-4ad5-9911-07e3de281971", "labels": ["niveau_detail::général"]}
## Contexte général
...
|===
| Processus métier | Finalité | Catégories de données manipulées | Classification | D | I | C | T

| Délivrance d’acte | _Produire un acte authentique_ | _identité, état civil, logs techniques_ | _élevée_ |_élevée_ | _Diffusion restreinte_ |  _moyenne_ | _moyenne_
|===
...
[cols="1,5"]
|===
| Légende | (D)isponibilité (I)ntégrité \(C)onfidentialité (T)race ou preuve. Donner le niveau requis : faible, moyen ou élevé
| Classification | Public, Interne, Diffusion restreinte, Confidentiel
|===

====

//🏷{"id": "3b714287-891e-4ea3-a7a4-17672caaf945", "labels": ["niveau_detail::général","contrainte"]}
## Contraintes

TIP: Limites / choix imposés sur la façon de concevoir ou de réaliser : technologies, processus, organisation, budget, réglementation.
...
====
====
Exemple 6 : Les modules de cette application suivent l'architecture microservices telle que définie dans le document de référence X.
====
====
...
//🏷{"id": "9352a89a-3f8b-4028-98d5-58fb970e01ef", "labels": ["exigence"]}
## Exigences
...
====
====
Exemple 2 : Le job X ne devra être lancé que si le job Y s’est terminé correctement sous peine de corruption de données.
====
====
Exemple 3 : Aucune autre instance du job X ne devra être lancée en parallèle sous peine de corruption de données.
====
...
====
====
Exemple 2 : Si le serveur SMTP ne fonctionne plus, les mails devront être persistés puis renvoyés automatiquement dès que le service est de nouveau disponible.
====
...
====
====
Exemple 2 : Les documents comptables doivent être conservés en ligne pendant une durée minimale définie, puis archivés à long terme, tout en garantissant que leur intégrité puisse être vérifiée de manière cryptographique dans le temps.
====
...
[cols="1e,1e"]
|===
| Nature de la Donnée | Durée maximale de rétention
...
//🏷{"id": "b269e65b-a8c7-4518-a861-5c6c17802869", "labels": ["solution","niveau_detail::général"]}
## Architecture cible
...
Ne pas faire figurer les systèmes d'infrastructure (serveur SMTP, dispositif de sécurité, reverse proxy, annuaires LDAP, …) qui sont du domaine de l’infrastructure. Mentionner en revanche les éventuels bus d'entreprise qui ont un rôle applicatif (orchestration de service par exemple).
====

====
Exemple : MesInfosEnLigne (MIEL) permet à une entreprise de récupérer par mail un document récapitulant toutes les informations que l’administration dispose sur elle. L'administration peut compléter ses données par celles d'une autre administration. Elle  est constituée de plusieurs modules indépendants (modules IHM, batchs ou services REST).
...
Pour chaque flux, donner le protocole, un attribut lecture/écriture/exécution et une description. Si le flux est asynchrone, le spécifier (dans l'exemple plus bas, l'appel est en pointillés).
...
//🏷{"id": "9ac6e5d2-e9a0-427e-ba12-27dedbd8ac4d", "labels": ["niveau::intermédiaire","migration"]}
...
//🏷{"id": "5f6e7d8c-9b0a-4f3e-8d7c-6b5a4e3d2c1f", "labels": ["niveau_detail::détaillé","archivage"]",link_to": ["4a3b2c1d-0e9f-4d8c-8b7a-6a5b4c3d2e1f"]}
### Archivage
...
//🏷{"id": "0a1b2c3d-4e5f-4061-82a3-b4c5d6e7f809", "labels": ["niveau_detail::détaillé"],link_to": ["fedcba98-7654-4321-8abc-0def12345678"]}
### Purges
...
_Mode: (L)ecture, (E)criture ou Lecture/Ecriture (LE), (A)ppel (vers un système stateless)_
