...
IMPORTANT: Les exigences sont situées dans la link:./vue-dimensionnement.adoc[vue dimensionnement].
...
Penser à l'exploitation des logs, surtout coté serveur. Se demander s'il sera possible d'en tirer profit en cas d'erreur en production au milieu de Mio voire Gio d'autres logs et de nombreux threads écrivant en parallèle.
====
...
* Dois-je fournir un moyen de purger des données, logs, caches, etc. ? 
