...
|Développement
|Déploiement continu (CD) pour les développeurs, donne une vision de la dernière version de l'ensemble des modules une fois les merge requests acceptés. Modules existants bouchonnés.
|Branche `develop` déployée à chaque commit
|Un seul

|Recette 
|Recette fonctionnelle car les testeurs
...
[TIP]
====
Lister ici les mesures d'infrastructure permettant de répondre aux exigences d'<<Ecoconception>>. 
...
[TIP]
====
Donner la politique générale de sauvegarde. Elle doit répondre aux <<Exigences de RPO>>. De même les dispositifs de restauration doivent être compatibles avec les <<exigences-disponibilite>> :
...
====
====
Exemple : Les batchs devront produire un rapport d'exécution à chaque lancement (avec des données de base comme le nombre d'éléments traités, la durée du traitement et tout indicateur pertinent).
====
